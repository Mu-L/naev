...
static int mission_meetReq( const MissionData *misn, int64_t faction,
                            const Spob *pnt, const StarSystem *sys );
static int mission_matchFaction( const MissionData *misn, int64_t faction );
 ...
 *    @param sys System to run on.
 */
void missions_run( MissionAvailability loc, int64_t faction, const Spob *pnt,
 ...
 */
Mission *missions_genList( int64_t faction, const Spob *pnt,
   ...
   do { /* load all the data */
      ...
      if ( xml_isNode( node, "faction" ) ) {
         if ( temp->avail.factions == NULL )
            temp->avail.factions = array_create( int64_t );
...
Commodity *missions_loadTempCommodity( xmlNodePtr cur )
{
   ...
   do {
      ...
      if ( xml_isNode( ccur, "illegalto" ) ) {
         int64_t f = faction_get( xml_get( ccur ) );
