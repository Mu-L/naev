...
return function ( t, max, params )
   ...
   t.patrol       = { f=spawn_patrol, w=math.max(0, -50+1.2*max*5) }
   t.loner_weak   = { f=spawn_loner_weak, w=100+0.5*max*5 }
   t.loner_strong = { f=spawn_loner_strong, w=max*5 }
   t.squad        = { f=spawn_squad, w=math.max(0, -150 + 1.2 * max*5) }
   t.capship      = { f=spawn_capship, w=math.max(0, capship_base + 1.70 * max*5) }
end
